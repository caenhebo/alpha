...
const STRIGA_CONFIG = {
  ...
  production: {
    ...
    secret: process.env.STRIGA_PROD_API_SECRET || process.env.STRIGA_PROD_SECRET
  }
}

const isProduction = process.env.NODE_ENV === 'production'
const config = isProduction ? STRIGA_CONFIG.production : STRIGA_CONFIG.sandbox
...
export async function initiateKYC(strigaUserId: string): Promise<{
  ...
  try {
    ...
    const kycUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'http://155.138.165.47:3018'}/kyc/verify?token=${response.token}`
